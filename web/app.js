/* rclone-web — vanilla JS app */
'use strict';

// ---- State ----
let csrfToken = '';
let providers = [];    // [{name, type, ...extra}]
let jobs = [];         // [{id, name, command, ...}]
let backends = [];     // rclone config providers JSON
let currentRun = null; // {id, jobId, dryRun, ...}
let pollTimer = null;
let idleTimer = null;
...
async function loadInitialData() {
  [jobs, providers, backends] = await Promise.all([
    api('GET', '/api/jobs'),
...
function renderDashboard() {
  ...
  if (!jobs.length) {
    tbody.innerHTML = '<tr><td colspan="4" class="px-5 py-6 text-center text-sm text-slate-400">No jobs yet. <a href="#" class="text-brand-600 hover:underline" onclick="showScreen(\'jobs\')">Add one</a>.</td></tr>';
    return;
  }
...
async function proceedWithRun() {
  ...
  try {
    const qs = pendingRunDryRun ? '?dryRun=true' : '';
    const data = await api('POST', `/api/jobs/${pendingRunJobId}/run${qs}`);
    if (!data) return;
    currentRun = { id: data.runId, jobId: pendingRunJobId };
...
async function stopRun() {
  ...
  try {
    await api('POST', `/api/runs/${currentRun.id}/stop`);
  } catch { /* ignore */ }
}
...
function renderProviderFields() {
  ...
  const backend = backends && backends.find(b => b.Name === type);
...
async function saveProvider() {
  ...
  try {
    if (editingProvName) {
      await api('PUT', `/api/providers/${editingProvName}`, { type, ...extra });
    } else {
