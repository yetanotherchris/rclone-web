...
function backendFieldHTML(opt, prefix) {
  ...
  if (isBlob) {
    ...
    input = `<div class="flex gap-2 items-stretch">
      ...
      <button type="button" class="pw-b64 rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-600 hover:bg-slate-50 whitespace-nowrap" data-target="pf-${esc(key)}" title="Base64-encode the current value">base64</button>
    </div>`;
...
function wirePasswordFields(container) {
  ...
  container.querySelectorAll('.pw-b64').forEach(btn => {
    btn.addEventListener('click', () => {
      const inp = document.getElementById(btn.dataset.target);
      if (!inp || !inp.value) return;
      inp.value = btoa(inp.value);
    });
  });
}
