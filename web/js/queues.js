...
function renderQueueJobList(jobIds) {
  ...
  list.querySelectorAll('.qf-up-btn').forEach(btn =>
    btn.addEventListener('click', () => moveQueueJob(Number(btn.dataset.idx), -1))
  );
  list.querySelectorAll('.qf-down-btn').forEach(btn =>
    btn.addEventListener('click', () => moveQueueJob(Number(btn.dataset.idx), 1))
  );
